...
EXP_PROPRIEDADE='d[0].properties = {resp_alfabetizado: Number(d[1].V078.replace(",", "."))}, d[0]'
...
EXP_ESCALA='z = d3.scaleThreshold().domain([0, 100, 400, 700, 1000]).range(d3.schemeBlues[5]), d.features.forEach(f => f.properties.fill = z(f.properties.resp_alfabetizado)), d'
...
ndjson-map \
  ...
  | geo2svg -n --stroke none -w 1000 -h 600 \
    > pb-chroropleth-alfabetizados.svg
